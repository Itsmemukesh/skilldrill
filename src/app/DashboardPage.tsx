        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {SKILLS.map((skill) => (
            <Card key={skill.id} hoverable={skill.ready} className={`flex flex-col ${!skill.ready ? 'opacity-65' : ''}`}>
              ...
              <h3 className="text-base font-bold text-text-main mb-2 line-clamp-1">
                {skill.ready ? (
                  <Link href={`/skills/${skill.id}`} className="hover:text-accent-base transition-colors">
                    {skill.name}
                  </Link>
                ) : (
                  skill.name
              ...
              {skill.ready && (
                <Link
                  ...
                  className="mt-2 text-xs font-mono text-text-sec hover:text-accent-base transition-colors text-center"
                >
                  Browse questions →
                </Link>
              )}
